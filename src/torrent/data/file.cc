...
File::set_priority(priority_enum t) {
  ...
  // Off: drop create/resize so unwanted files are not materialized. Boundary
  // writes re-queue create in FileList::create_chunk_part when needed.
  // Leaving off: restore create/resize so download can create the file.
  if (t == PRIORITY_OFF) {
